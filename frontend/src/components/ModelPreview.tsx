...
function Model({ url, showSkeleton, isRotating, resetTrigger }: { url: string; showSkeleton?: boolean; isRotating: boolean; resetTrigger: number }) {
      ...
      bonesRef.current.forEach((bone, index) => {
        ...
        if (cone) {
          ...
          if (index === 0 && Math.random() < 0.01) { // Log occasionally
            fetch('http://127.0.0.1:7242/ingest/b89e3547-7de1-4c89-94c3-53229b5a026e',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ModelPreview.tsx:35',message:'Cone position update in useFrame',data:{boneIndex:index,worldPosX:worldPos.x,worldPosY:worldPos.y,worldPosZ:worldPos.z,localPosX:localPos.x,localPosY:localPos.y,localPosZ:localPos.z,sceneRotationY:gltf.scene.rotation.y,sceneScaleX:gltf.scene.scale.x},timestamp:Date.now(),sessionId:'debug-session',runId:'rotation-debug',hypothesisId:'G'})}).catch(()=>{});
          }
          // #endregion
        }
      });
  ...
  useEffect(() => {
    ...
    if (gltf.scene && meshRef.current) {
      ...
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/b89e3547-7de1-4c89-94c3-53229b5a026e',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ModelPreview.tsx:47',message:'Model scaling calculation',data:{maxDim,scale,sizeX:size.x,sizeY:size.y,sizeZ:size.z,centerX:center.x,centerY:center.y,centerZ:center.z},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
      ...
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/b89e3547-7de1-4c89-94c3-53229b5a026e',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ModelPreview.tsx:58',message:'Scale stored in ref (POST-FIX)',data:{storedScale:modelScaleRef.current,calculatedScale:scale,sceneScaleX:gltf.scene.scale.x,sceneScaleY:gltf.scene.scale.y,sceneScaleZ:gltf.scene.scale.z},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
    }
  }, [gltf]);
        ...
        gltf.scene.traverse((object) => {
          if (object instanceof THREE.SkinnedMesh && object.skeleton) {
            ...
            skeleton.bones.forEach((bone, boneIndex) => {
              ...
              // #region agent log
              if (boneIndex === 0 || boneIndex === Math.floor(skeleton.bones.length / 2)) {
                fetch('http://127.0.0.1:7242/ingest/b89e3547-7de1-4c89-94c3-53229b5a026e',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ModelPreview.tsx:110',message:'Bone position and length (SPAN-FULL-LENGTH)',data:{boneIndex,coneRadius,coneHeight,boneLength,modelScale,boneWorldPosX:worldPos.x,boneWorldPosY:worldPos.y,boneWorldPosZ:worldPos.z,coneLocalPosX:localPos.x,coneLocalPosY:localPos.y,coneLocalPosZ:localPos.z},timestamp:Date.now(),sessionId:'debug-session',runId:'span-full-length',hypothesisId:'C'})}).catch(()=>{});
              }
              // #endregion
            ...
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/b89e3547-7de1-4c89-94c3-53229b5a026e',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ModelPreview.tsx:140',message:'Skeleton group added (SIMPLIFIED)',data:{coneCount:skeletonGroup.children.length,skeletonGroupScaleX:skeletonGroup.scale.x,skeletonGroupScaleY:skeletonGroup.scale.y,skeletonGroupScaleZ:skeletonGroup.scale.z,sceneScaleX:gltf.scene.scale.x,sceneScaleY:gltf.scene.scale.y,sceneScaleZ:gltf.scene.scale.z},timestamp:Date.now(),sessionId:'debug-session',runId:'simplified',hypothesisId:'F'})}).catch(()=>{});
            // #endregion
